...
def speculate_attention(hidden, p_w_q, p_k_c, n_head, alpha, max_num_kv):
    ...
    Returns:
        prefetch_idx: Indices of critical KV cache tokens for each head and batch (n', 1, bh)
    """
    # print(f"hidden: {hidden.shape}")  # (4, 1, 4097)
    # print(f"p_w_q: {p_w_q.shape}")  # (800, 4097)
    # print(f"p_k_c: {p_k_c.shape}")  # (338, 128, 25)
    pkc_shape = p_k_c.shape
    b = hidden.shape[0]
    p_k_c_reshaped = p_k_c.view(pkc_shape[0], b, -1, pkc_shape[-1])  # (338, 4, 32, 25)

    p_q = F.linear(hidden, p_w_q, bias=None)
    p_q = p_q.view(b, 1, n_head, -1)  # d due to hidden
    p_q = p_q.permute(0, 2, 1, 3).reshape(b * n_head, 1, -1)  # (128, 1, 25)

    p_attn = torch.bmm(p_q, p_k_c.permute(1, 2, 0))
    max_ = torch.max(p_attn, dim=-1)[0]  # (128,)
    thr_ = (max_ - alpha).unsqueeze(-1).repeat(1, 1, p_attn.shape[-1])  # (128, 1, 389)
    count = torch.where(
    ...
    # print(f"p_attn: {p_attn.shape}")  # (128, 1, 389)
    # print(f"prefetch_idx: {prefetch_idx.shape}")
    # print(f"thr_: {thr_.shape}")
    # print(f"count: {count.shape}")
    # print("====================")
    p_attn = p_attn.permute(2, 1, 0)

    return prefetch_idx


# [x] For GQA
def select_kv_gqa(prefetch_idx, k_cache, v_cache):
    """Selects and aggregates critical KV caches using speculated indices
    ...
    Args:
        prefetch_idx: Indices of critical KV cache tokens for each head and batch (n', 1, bh_kv)
        k_cache: Key cache (n, bh_kv, d)
        v_cache: Value cache (n, bh_kv, d)

    Returns:
        selected_k: selected key cache (n', bh_kv, d)
        selected_v: selected value cache (n', bh_kv, d)
    """
...
# [x] For GQA
def speculate_attention_gqa(
    hidden, p_w_q, p_k_c, q_head_num, kv_head_num, alpha, max_num_kv
):
    """Speculates the indices of the critical KV caches of next attention layer.
    ...
    Args:
        hidden: Hidden states of layer i (b, 1, D)
        p_w_q: Partial query weight (D', D) (where D is bh_q * d = 32 * 128)
        p_k_c: Partial key cache (n, bh_kv, d') where

        Note that bh_q * d' == D'? h_q * d' == D'
    ...
    pkc_shape = p_k_c.shape
    b = hidden.shape[0]
    p_k_c_reshaped = p_k_c.view(
        pkc_shape[0], b, -1, pkc_shape[-1]
    )  # (338, b, h_kv, 25)
    p_k_c_repeated = p_k_c_reshaped[:, :, None, :, :].expand(
        pkc_shape[0], b, kv_head_num, q_per_kv, pkc_shape[-1]
    )
    p_k_c_repeated = p_k_c_repeated.reshape(
        pkc_shape[0], b * kv_head_num * q_per_kv, pkc_shape[-1]
    )  # (338, bh_q, 25)

    p_q = F.linear(hidden, p_w_q, bias=None)
    p_q = p_q.view(b, 1, q_head_num, -1)
    p_q = p_q.permute(0, 2, 1, 3).reshape(b * q_head_num, 1, -1)

    p_attn = torch.bmm(p_q, p_k_c_repeated.permute(1, 2, 0))

    # Don't need to modify how many kv are prefetched
    ...
    # Select the topk by considering mean of a group
    """
    In MHA, prefetch_idx: (n', 1, bh_q)
    In GQA, prefetch_idx: (n', 1, bh_kv)
    """
    p_attn = p_attn.view(b, kv_head_num, q_per_kv, 1, -1)  # (b, h_kv, q_per_kv, 1, 389)
    p_attn = torch.mean(p_attn, dim=2, keepdim=True)
    p_attn = p_attn.reshape(b * kv_head_num, 1, -1)  # (bh_kv, 1, n')
